...
wandb.init(project="LC50-ordmlp", entity="soyoung")
...
def main(seed_):
    ...
    train_x = tf.cast(train_fingerprints, tf.float32)
    tf_train_y = tf.cast(train_y, tf.int32)
    test_x = tf.cast(test_fingerprints, tf.float32)
    tf_test_y = tf.cast(test_y, tf.int32)
    ...
    result_ = {
        ...
        'tau': stats.kendalltau(test_y, mlp_pred).correlation
    }
...
pd.DataFrame(result).to_csv('../test_results/ordmlp.csv', header = True, index = False)
wandb.finish()
