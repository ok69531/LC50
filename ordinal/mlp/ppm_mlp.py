...
def main(seed_):
    ...
    train_x = tf.cast(train_ppm_fingerprints, tf.float32)
    train_y = tf.cast(train_ppm_y, tf.int32)
    test_x = tf.cast(test_ppm_fingerprints, tf.float32)
    test_y = tf.cast(test_ppm_y, tf.int32)
    
    
    params_dict = {
        'learning_rate': [0.01, 0.001, 0.0005],
        'epochs': [500, 1000, 5000, 10000, 30000, 50000]
    }
    ...
    max_tau_idx = mlp_result.val_tau.argmax(axis = 0)
    best_params = mlp_result.iloc[max_tau_idx][:3].to_dict()
    ...
    test_loss = scc(test_y, mlp_pred_prob).numpy()
    ...
    result_ = {
        ...
        'test_loss': test_loss, 
        'precision': precision_score(test_ppm_y, mlp_pred, average = 'macro'), 
        'recall': recall_score(test_ppm_y, mlp_pred, average = 'macro'), 
        'f1': f1_score(test_ppm_y, mlp_pred, average = 'macro'), 
        'accuracy': accuracy_score(test_ppm_y, mlp_pred),
        'tau': stats.kendalltau(test_ppm_y, mlp_pred).correlation
    }
...
pd.DataFrame(result).to_csv('../test_results/ppm_mlp.csv', header = True, index = False)
wandb.finish()
