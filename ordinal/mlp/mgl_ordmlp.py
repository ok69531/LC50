...
wandb.init(project="LC50-mgl-ordmlp", entity="soyoung")
...
def main(seed_):
    ...
    train_x = tf.cast(train_mgl_fingerprints, tf.float32)
    train_y = tf.cast(train_mgl_y, tf.int32)
    test_x = tf.cast(test_mgl_fingerprints, tf.float32)
    test_y = tf.cast(test_mgl_y, tf.int32)
    ...
    result_ = {
        ...
        'tau': stats.kendalltau(test_mgl_y, mlp_pred).correlation
    }
...
pd.DataFrame(result).to_csv('../test_results/mgl_ordmlp.csv', header = True, index = False)
wandb.finish()
